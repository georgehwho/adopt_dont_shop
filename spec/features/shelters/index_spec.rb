...
RSpec.describe 'the shelters index' do
  ...
  it 'has a link to delete each shelter' do
    ...
    within "#shelter-#{@shelter_1.id}" do
      expect(page).to have_selector(:link_or_button, "Delete #{@shelter_1.name}")
    end

    within "#shelter-#{@shelter_2.id}" do
      expect(page).to have_selector(:link_or_button, "Delete #{@shelter_2.name}")
    end

    within "#shelter-#{@shelter_3.id}" do
      expect(page).to have_selector(:link_or_button, "Delete #{@shelter_3.name}")
    end
